...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $sqlTanggal = "UPDATE dbo.adminPusat_konfirmasi SET tanggal_adminJurusan_konfirmasi = GETDATE() WHERE nim = ?";
    $params2 = [$nim];

    $stmt2 = sqlsrv_query($conn, $sqlTanggal, $params2);

    if ($stmt && $stmt2) {
        header("Location: ../foto_ijazah.php?message=Status+berhasil+diperbarui!&type=success");
        exit();
    } else {
